                        ...
                        <tr>
                            ...
                            <td>
                                <form class="d-flex" method="POST" action="{{ route('peta.delete', $pet->id) }}">
                                    @csrf
                                    @method('delete')
                                    <button class="btn btn-danger" onclick="return confirm('anda yakin ingin menghapus {{ $pet->nama_tempat }} ini secara permanen?');event.preventDefault();
            ...
            <div class="card-body">
                <form action="{{ route('peta.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input type="text" id="nama" class="form-control  @error('nama_tempat') is-invalid @enderror"
                            name="nama_tempat" value="{{ old('nama_tempat') }}" placeholder="Nama Lokasi">
                        @error('nama_tempat')
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input type="text" id="lat" class="form-control  @error('lat') is-invalid @enderror" name="lat"
                            value="{{ old('lat') }}" placeholder="0.7964421975787448">
                        @error('lat')
                        <span class="invalid-feedback" role="alert">
                            <strong class="text-danger">{{ $message }}</strong>
                        </span>
...
@section('scrip')
<script>
    var map = L.map('map').setView([1.057314, 127.462270], 17);
